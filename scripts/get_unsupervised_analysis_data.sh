...
if [[ ! -d ~/summer_school_vms ]]; then
    git clone https://github.com/haam-community/summer_school_vms.git & git lfs pull --include "data/unsupervised_analyses/*.tar.gz"
else
    cd ~/summer_school_vms
    git lfs pull --include "data/unsupervised_analyses/*.tar.gz"
    cd ~
fi
